  ...
  <v-layout fill-height>
    <v-navigation-drawer permanent>
      ...
      <template v-slot:append>
        <div class="pa-2">
          <v-btn block depressed color="secondary" @click="logout">
            Logout
          </v-btn>
        </div>
      </template>
  ...
  export default {
    ...
    methods: {
      ...
      setNavBar(){
        ...
        }else if(this.log.type == 'admin'){
          this.items = [
            { title: 'Dashboard', icon: 'mdi-view-dashboard' , href: '/admin'},
            { title: 'Booking Transaction', icon: 'mdi-calendar-month', href: '/admin/booking' },
            { title: 'Vehicle Listings', icon: 'mdi-car-multiple', href: '/admin/vehicle' },
            { title: 'Financial Report', icon: 'mdi-account-cash', href: '/admin/financial' }
          ]
        }else{
          this.items = []
        }
