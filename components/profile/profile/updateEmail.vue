      ...
      <v-card class="rounded-xl">
        <v-card-text class="pa-3">
          ...
          >
                  ...
                  <v-layout column>
                    ...
                    <v-flex>
                      <v-text-field
                        ...
                        persistent-placeholder
                        class="rounded-xl"
                        :error-messages="errors ? errors.confirm_email ? errors.confirm_email :'':''"
                      />
                    </v-flex>
                  </v-layout>
                </v-card-text>
            ...
            >
              Authentication
              <small>Your verification code is sent via email to <small class="font-weight-bold">{{ email ? email : 'N/A' }}</small></small>
            </v-stepper-step>
...
export default {
  ...
  methods: {
    ...mapActions('users', ['SEND_EMAIL_UPDATE_OTP', 'USER_UPDATE_EMAIL']),
    ...
    async authenticate(){
      ...
      if(validation.error == true){
        ...
        await this.USER_UPDATE_EMAIL(payload).then(data => {
          this.$swal.fire({
            title: `Email has been renewed.`,
            text: '', 
            icon: 'success',
    ...
    async sentOtp(){
      let payload = {
        ...
        confirm_email : this.confirm_email ? this.confirm_email : null
      }
      let validation = this.fieldsValidation(payload)
      if(validation.error == true){
        this.errors = validation.errors
        ...
        if(this.email == this.confirm_email){
          ...
          this.$delete(this.errors, 'confirm_email')
          await this.SEND_EMAIL_UPDATE_OTP(payload).then(data => {
            this.e1 = 2
          }).catch(response => {
            this.errors = response.response.data.errors
          }) 
        }else{
          this.$set(this.errors, 'email', ['Email did not match']);
          this.$set(this.errors, 'confirm_email', ['Email did not match']);
        }
      }
    },
    openDialog(){
